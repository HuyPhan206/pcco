...
<head>
    ...
    <title>Thêm sản phẩm - TTG Shop</title>
...
@section('content')
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" id="productForm">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="category_id" name="category_id" required>
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}" {{ old('category_id', $categories->first()->id ?? null) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('discount_price') is-invalid @enderror" id="discount_price" name="discount_price" value="{{ old('discount_price') }}" step="0.01">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="stock" name="stock" value="{{ old('stock', 0) }}" min="0" step="1" required>
        </div>
        <div class="mb-3">
